...
- Added an optional `desiredDeliveryDate` field to the `Checkout` and `Order` types, and a new `checkoutDesiredDeliveryDateUpdate` mutation to set or clear it. The date must be between tomorrow and 30 days from now, calculated in UTC; otherwise the mutation returns a `DESIRED_DELIVERY_DATE_OUT_OF_RANGE` error. The value is copied to the `Order` when the checkout is completed.
